...
export default function EditInventoryPage() {
      ...
      .then((items) => {
        ...
        if (p?.variants && Array.isArray(p.variants)) {
          setVariants(p.variants.map((v: any) => ({
            ...
            _imageFile: null,
            _imagePreview: v.image_url || null,
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    for (const v of variants) {
      ...
      if (v._imageFile) {
        variantImageUrl = await uploadInventoryImage(v._imageFile, product.id);
      }
      finalVariants.push({
        color_name: v.color_name,
        color_hex: v.color_hex,
        product_name: v.product_name || product.product_name,
        product_sku: v.product_sku || product.product_sku,
        stock_quantity: Number(v.stock_quantity) || Number(product.stock_quantity),
        image_url: variantImageUrl,
      });
    }

    // Update mainImage logic: fallback to variant 0 if needed
    if (finalVariants.length > 0) {
      mainImageUrl = finalVariants[0]?.image_url || mainImageUrl;
    }
                  ...
                  onClick={() =>
                    setVariants([
                      { 
                        ...
                        _imageFile: null, 
                        _imagePreview: product.main_image_url, 
