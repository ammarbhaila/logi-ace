...
export default function GlobalInventoryLogsPage() {
    ...
    useEffect(() => {
        const fetchRoleAndLogs = async () => {
            try {
                ...
                console.error('[DEBUG] Failed to fetch global inventory logs:', err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredLogs = inventoryLogs.filter((log) => {
        const searchStr = `${log.product_name} ${log.product_sku || ''} ${log.action} ${log.performed_by}`.toLowerCase();
    ...
    const handleExport = () => {
        ...
        downloadCSV(exportData, 'inventory_activity_logs');
    };
            ...
            <div className="max-w-[1400px] mx-auto transition-all duration-300">
                                ...
                                {filteredLogs.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-20 text-center text-gray-500 ">
                                            {searchTerm ? 'No inventory logs matching your search.' : 'No inventory logs found in the system.'}
                                        </td>
                                    </tr>
                                ) : (
                                    filteredLogs.map((log) => (
                                        <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-5 py-2 text-[13px] font-semibold text-[#1e40af] border-r border-gray-100">
                                                {log.product_name}
                                            </td>
                                            <td className="px-5 py-2 text-[13px] text-gray-600 border-r border-gray-100 whitespace-nowrap">
                                                {log.product_sku || '-'}
                                            </td>
                ...
                <div className="flex items-center justify-between mt-7">
                    <div className="text-[13px] text-gray-500 font-medium">
                        Showing <span className="text-gray-800">{filteredLogs.length}</span> of <span className="text-gray-800">{inventoryLogs.length}</span> logs
                    </div>
                    {/* Pagination buttons hidden as requested in similar pages */}
                </div>
            </div>
        </div>
    );
