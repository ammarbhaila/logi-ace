...
async function sendOverdueReminderEmail(orderId: string, orderData: any, cartItems: any[], shippedDate: string) {
              ...
              <table width="650" style="${STYLES.container}" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td style="padding:22px 24px;border-bottom:1px solid #eef3f7;">
                    <div style="font-size:20px;font-weight:500;color:#000000;">
                      Overdue Reminder (Order #${orderId})
                    </div>
                ...
                <tr>
                  <td style="padding:22px 24px;font-size:14px;color:#334b59;line-height:20px;">
                    This is a message from SHI UC HUB to remind you that Order #${orderId} is now shipped for a period of 45 days against its 30-day trial period.<br><br>
                    Please remind your customer to use the return label that was provided with the order & ship back their devices & accessories, ASAP. You can also click on the button to view order details and download return label again if needed.<br><br>
  ...
  const subject = `Overdue Reminder | Order #${orderId} (${orderData.customer_company_name})`;
  ...
  await transporter.sendMail({
    ...
    // cc: "arman@works360.com",
    subject,
    html: buildHtml(),
    attachments: [{ filename: 'logo.png', path: logoPath, cid: 'logoimg' }],
  });

  console.log(`[DEBUG] Overdue reminder email sent for order #${orderId} to ${sendTo}`);
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[DEBUG] Processing Overdue Reminder email for order #${orderId}`);

    await sendOverdueReminderEmail(orderId, orderData, cartItems, shippedDate || 'N/A');

    // Update the database to mark as sent
    await supabaseAdmin
      .from('checkout_requests')
      .update({ overdue_reminder_sent: true })
      .eq('id', orderId);

    return NextResponse.json({ success: true, message: 'Overdue Reminder email sent and recorded' });
  } catch (error: any) {
    console.error('[DEBUG] Overdue Reminder Email API Error:', error);
